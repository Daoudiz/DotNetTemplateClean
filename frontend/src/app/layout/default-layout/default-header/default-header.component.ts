...
import { AuthService } from '../../../services/user/auth.service'; // Ajustez le chemin selon votre structure
...
@Component({
  ...
  imports: [ContainerComponent, HeaderTogglerDirective, SidebarToggleDirective, IconDirective, 
    HeaderNavComponent, NavItemComponent, NavLinkDirective, RouterLink, 
...
export class DefaultHeaderComponent extends HeaderComponent {
  ...
  // Injection de vos services pour le logout
  ...
  public onLogout(): void {
    ...
    this.router.navigate(['/login']); // Redirige vers la page blanche de login
  }
  ...
  getInitial(): string {
    ...
    // On récupère le premier caractère alphanumérique
    if (name && name.length > 0) {
      return name.trim().charAt(0).toUpperCase();
    }
    return '?'; // Fallback si le userName est vide
  }
//#endregion
